...
- Android Auto media support now exposes a Media3 `MediaLibraryService` (`JellyMixMediaLibraryService`) instead of the legacy platform media-browser manifest path. The service uses Media3 ExoPlayer with media audio attributes/audio focus, advertises the full Player command set for Auto controls, validates callers for Gearhead and JellyMix, logs rejected caller package names plus SHA-256 signing digests for AI Box diagnosis, serves async browse/search results for Playlists, Albums, Artists, Recently Added, and Favorites from Jellyfin/cached metadata, resolves playable MediaItems to Jellyfin stream URLs, and returns a readable connection item when unauthenticated instead of an empty tree.
- Mixes tab crash/jank is mitigated by keeping full-library promotion out of the Mixes tab and ranking generated/vibe mixes from a bounded 900-track recommendation source that blends visible cached tracks, liked/local-play/long-listen signals, a library-wide stride sample, and tail items. Library remains the only tab that promotes the full cached Jellyfin dataset into the active Compose model.
- Background playback persistence is hardened for the phone path: active Jellyfin playback now keeps a lightweight foreground media service alive while the Activity-owned ExoPlayer is playing, playback position is saved every few seconds plus on pause/stop/clear, startup restores the saved current track position, prepared ExoPlayer streams seek to that saved position before playing, notification media-session state advertises the real position, and the widget/notification fallback service also resumes from the saved position when it has to restart playback after the Activity is gone.
- Offline listening is enabled for individual Jellyfin tracks. Track rows in Mixes/Vibes/Yours/Library and the Now Playing overflow can save/remove an app-private audio copy under `filesDir/offline-audio`; saved track ids persist locally, downloads show Saving/Saved state, playback resolves downloaded files before Jellyfin streams, and phone, widget/notification fallback, and Media3 Android Auto playback all prefer the local file when present. No shared-storage permission is required because offline audio stays private to JellyMix.
...
- Playback responsiveness pass: live playback progress moved out of the giant `JellyMixState` into a tiny mutable float read only by progress bars, skip/queue/DJ track transitions now yield one frame before Jellyfin player setup so UI updates land immediately, Now Playing no longer triggers full Mixes-tab generation, queue/autoplay previews cap their source windows, and the next queued Jellyfin track preloads while music is playing for faster skips.
- Phone playback now uses AndroidX Media3/ExoPlayer instead of framework `MediaPlayer`, including the main Jellyfin stream, prepared next-track handoff, and crossfade player. This gives the phone app a real streaming playback engine with asynchronous preparation, better buffering behavior, and less UI-thread pressure during skip/start/Now Playing interactions. Android Auto and widget services still use their existing isolated playback paths.
- Phone skip/start handoff is crash-hardened with a monotonic playback request token and pending-player slot: stale Exo callbacks are ignored/released, the old active player stays alive until the new stream is ready, player release calls are guarded, and invalid Exo audio session ids no longer reach Android's `Visualizer`.
- Manual skip/previous is faster: normal queue skips no longer sort/rank the full Jellyfin library, previous uses a lightweight fallback when no queue exists, manual switches interrupt old playback immediately, and next-track preloading only targets real upcoming queue items.
...
- Now Playing progress is hardened for Jellyfin streams that report unset/late player duration: elapsed and remaining time now fall back to the track metadata duration while still using the live ExoPlayer position, and playback progress is refreshed as soon as the player becomes ready.
...
- Swipe-down media controls now publish cleaner title/artist/album metadata, a decoded/cached Jellyfin artwork bitmap for launchers that ignore remote art URIs, duration plus seek state for the system seek bar, notification seek routing into the live ExoPlayer path, and expanded Like/Previous/Play/Next/Shuffle actions while keeping the compact card focused on Previous/Play/Next.
- Home, Mixes, and Library now consume cached tab data models from `JellyMixDerivedDataCache` instead of rebuilding ranked mixes, vibe mixes, recent plays, library grouping, and discovery rows directly in the Compose root on every state copy. Library and Vibe searches use local debounced drafts so typing does not immediately filter or regroup large Jellyfin libraries on every keypress. Mix and Vibe cards were tightened into scannable playlist-entry cards with artwork, track counts, top-artist summaries, one primary Play action, outlined Shuffle, and fewer embedded preview rows.
- Discover no longer exposes Jarvis as a front-facing chat/prompt screen. The assistant logic now stays behind the scenes and feeds automatic custom playlists built from likes, replays, long listens, skips, recent history, current-track continuity, and inferred audio features. Now Playing also replaces the visible Jarvis mode card with a neutral personalized autoplay card.
...
- Playback responsiveness release: `https://github.com/BadBagger/jellymix/releases/tag/v0.1.30-playback-responsiveness`
- ExoPlayer phone playback release: `https://github.com/BadBagger/jellymix/releases/tag/v0.1.31-exoplayer-phone-playback`
- ExoPlayer skip crash fix release: `https://github.com/BadBagger/jellymix/releases/tag/v0.1.32-exoplayer-skip-crash-fix`
...
- Media3 Android Auto release: `https://github.com/BadBagger/jellymix/releases/tag/v0.1.39-media3-android-auto`
- Mixes crash fix release: `https://github.com/BadBagger/jellymix/releases/tag/v0.1.40-mixes-crash-fix`
- Background playback resume release: `https://github.com/BadBagger/jellymix/releases/tag/v0.1.41-background-resume`
- Offline listening release: `https://github.com/BadBagger/jellymix/releases/tag/v0.1.42-offline-listening`
- Notification polish release: `https://github.com/BadBagger/jellymix/releases/tag/v0.1.43-notification-polish`
